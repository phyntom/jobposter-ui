...
const schema = z.object({
   ...
   company: z.object({
      ...
      contactPhone: z.string().regex(phoneValidator, { message: 'Invalid phone number' }),
      website: z.string().url({ message: 'Invalid website URL' }),
   }),
});
type JobIForm = z.infer<typeof schema>;

const types: Array<string> = ['Full Time', 'Part Time', 'Contract', 'Freelance', 'Internship'];
...
export default function NewJob() {
   ...
   const onSubmit: SubmitHandler<JobIForm> = async (data) => {
      ...
      const res = await fetch('https://jobposter-api.onrender.com/api/jobs', {
         method: 'POST',
            ...
            <form className='flex flex-col gap-8 mb-8' onSubmit={handleSubmit(onSubmit)}>
               ...
               <div className='flex flex-col gap-4'>
                  ...
                  {errors.description?.message !== undefined ? (
                     ...
                     <p className='text-sm text-muted-foreground'>
                        Provides a detailed overview of the job responsibilities, qualifications, or
                        any information that applicants should know about the position
                     </p>
                  )}
               </div>
               <div className='flex flex-col gap-4'>
                  ...
                  {errors.salary?.message !== undefined ? (
                     ...
                     <p className='text-sm text-muted-foreground'>
                        Please indicates the compensation or pay range for the job position
                     </p>
                  )}
               </div>
               <div className='flex flex-col gap-4'>
                  ...
                  {errors.tags?.message !== undefined ? (
                     ...
                     <p className='text-sm text-muted-foreground'>
                        Please enter the skills tags ex: JavaScript, React, Node.js end press space
                        to add
                     </p>
                  )}
               ...
               <div className='flex flex-col space-y-4'>
                  ...
                  {errors.isRemote?.message !== undefined ? (
                     <p className='text-sm text-destructive'>{errors.isRemote.message}</p>
                  ) : (
                     <p className='text-sm text-muted-foreground'>This job can be done remotely</p>
                  )}
               </div>
               <Separator className='my-4' />
               <div className='space-y-1'>
               ...
               <div className='flex justify-between gap-4'>
                  <div className='flex flex-col gap-2 basis-1/2'>
                     ...
                     <Input type='text' {...register('company.contactEmail')} className='flex-1' />
                  ...
                  <div className='flex flex-col gap-2 basis-1/2'>
                     ...
                     <Input type='tel' {...register('company.contactPhone')} className='flex-1' />
                     {errors?.company?.contactPhone?.message !== undefined ? (
                        <p className='text-sm text-destructive'>
                           {errors?.company?.contactPhone?.message}
                        </p>
                     ) : (
                        <p className='text-sm text-muted-foreground'>Enter company contact phone</p>
                     )}
                  </div>
               </div>
